...
def get_halo_particle_data(base_halo_data,snap2,ihalo,add_subparts_to_fofs=True):
    ...
    dump_pickle(path=f'vis_data/coordinates/ihalo{str(ihalo).zfill(6)}_snap{str(snap).zfill(3)}_current_xyz.dat',data=ihalo_Coordinates_snap2)
    dump_pickle(path=f'vis_data/coordinates/ihalo{str(ihalo).zfill(6)}_snap{str(snap).zfill(3)}_previous_xyz.dat',data=ihalo_Coordinates_snap1)
